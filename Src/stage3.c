...
static struct sprite_bird red_bird, blue_bird, green_bird;
static CP_Image red_bird_Forward[4], blue_bird_Forward[4], green_bird_Forward [4];
static CP_Image red_bird_Backwards[4], blue_bird_Backwards[4], green_bird_Backwards[4];
...
void stage3_init(void)
{
	...
	background = CP_Image_Load("Assets/background 3.png");
...
{	
	...
	if (gIsPaused == TRUE)
	{
		...
		if (CP_Input_MouseClicked(MOUSE_BUTTON_LEFT))
		{
			if (IsAreaClicked(1200, 800, 150, 60, CP_Input_GetMouseX(), CP_Input_GetMouseY()))
			{
				CP_Engine_SetNextGameState(stage4_init, stage4_update, stage4_exit);
				printf("Moving on to stage 2\n");
			}
			if (IsAreaClicked(600, 800, 150, 60, CP_Input_GetMouseX(), CP_Input_GetMouseY()))
			{
	...
	else
	{
		if (CP_Input_MouseClicked(MOUSE_BUTTON_LEFT)) //Logic for red_bird collision theory
		{
			...
			if (IsCircleClicked(red_bird.x, red_bird.y, 100, CP_Input_GetMouseX(), CP_Input_GetMouseY()))
			{
				...
				CP_Sound_Play(Score);
				printf("Counter is %d\n", counter);
				printf("Ammunition left is %d\n", initial_ammunition);
				ammunition_used++;
				printf("Ammunition used is %d\n", ammunition_used);
